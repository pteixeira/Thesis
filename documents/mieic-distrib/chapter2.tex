...
\section{Developments, Applications and Services}\label{sec:developments-apps-services}
...
%[03:07] <Sardines> the way on how different instances from the same image are configured (ip, authentication, etc etc etc)
%[03:07] <Sardines> images*
%[03:08] <zaitcev> So, it's like what Audrey does?
%[03:08] <Sardines> audrey?
...
%[03:09] == dtroyer_zzz has changed nick to dtroyer
%[03:11] <Sardines> cloud-init only works on ubuntu based images, right?
%[03:11] <smoser> :)
...
%[03:12] <harlowja> that is the hope, :-p
%[03:12] <smoser> Sardines, it does exist in fedora at the moment, but in a limited fashion
%[03:12] == pixelbeat [~padraig@109.78.86.189] has joined #openstack-dev
%[03:13] <smoser> Sardines, and there is cloud-init in debian sid right now, although there is work to be don there also.
...
%[03:14] <Sardines> ill use the ubuntu one for now :) working on my master thesis, need to cover this part of the process :p got any links that i can follow so i can get some more richness on the document? :D
...
%[03:15] <harlowja> ha
%[03:16] <Sardines> think im sticking to the surface for now, time is running short xD
%[03:17] <Sardines> actually, can you link me to the capability docs?
%[03:18] <harlowja> hmmm, ok, so hmmm, the capabilities, are sort of in that main one, http://bazaar.launchpad.net/~cloud-init-dev/cloud-init/trunk/files/head:/doc/examples/ + code unless there is another better place
...
%[03:19] <Sardines> thanks a million :) you have been of great help :D
%[03:20] <harlowja> if that isn't so easy, i can try to explain it all, haha, smoser or i should be able to do that :-p
%[03:20] <harlowja> http://bazaar.launchpad.net/~harlowja/cloud-init/rework/files might be easier to follow, basically starting in stages.py/init and then to the stages.py/transforms but don't expect that branch to work yet, lol
%[03:20] <harlowja> but for refereence it might be better, idk
%[03:21] <harlowja> sorry, http://bazaar.launchpad.net/~harlowja/cloud-init/rework/files/head:/cloudinit/, not the main dir
%[03:22] <Sardines> once again, thanks a million :)
...
%[03:24] == Facefox [~facefox@pool-74-111-197-200.lsanca.fios.verizon.net] has quit []
%[03:25] <Sardines> thank you very much once again :)
%[03:26] <Sardines> i want to cite both of you in the thesis, do you guys have any preference on how to do it? :D
%[03:26] <Sardines> preferences*
...
%[03:31] <Sardines> ahah ok :D
%[03:31] <Sardines> again, thanks a million for the help :)
