...
\section{Virtualization and Virtual Machines}\label{sec:virt}
...
\section{Developments, Applications and Services}\label{sec:dev}
...
\section{Software Packages}\label{pack}

Webopedia\footnote{http://www.webopedia.com} describes \textit{Software Packages} as:
\begin{quote}``A special method of distributing and installing software (or software upgrades) to a computer.''~\cite{webopedia-packs}
\end{quote}
...
\subsection{Debian Packages}\label{pack:debian}

There are two types of \textit{Debian packages}:

\begin{itemize}
\item Binary packages - Contain executables, configuration files, copyright information and other documentation.The packages are distributed in a \textit{Debian}-specific archive format and they are usually distinguished by having a `.deb' file extension. Then can be unpacked by using the \textit{Debian} utility \textit{dpkg};
\item Source packages - A `.dsc' file which describes the source package, a `.orig.tar.gz' file which contains the original unmodified source in \textit{gzip}-compressed tar format and a `.diff.gz' file that contains the \textit{Debian}-specific changes to the original source.
\end{itemize}

Installing the software via the package system uses ``dependencies'' which are carefully designed by the package maintainers. They are documented in a special \textit{control} file associated with each package.

\textit{Debian}'s packaging tools can be used to:
\begin{itemize} 
\item Manipulate and manage packages or parts of packages;
\item Administer local overrides of files in a package;
\item Aid developers in the construction of package archives;
\item Aid users in the installation of packages which reside one a remote FTP site.\cite{pkg-basics}
\end{itemize}

Working with packages in \textit{Debian} uses three main utilities:
\begin{itemize}
\item \textbf{apt} - Advanced Package Tool - Main package manager in \textit{Debian} systems used for retrieving/installing, removing or searching for packages;
\item \textbf{dpkg} - package manager for \textit{Debian} - Predecessor of \textit{``apt''}. Tool to install, build, remove and manage \textit{Debian} packages. Controlled entirely via command line parameters.\cite{manpages-dpkg}
\item \textbf{dselect} - menu driven front-end that uses both \textit{``apt''} and \textit{``dpkg''}.
\end{itemize}

\subsubsection{Repositories}\label{pack:debian:repos}

A \textit{Debian} repository is a set of \textit{Debian} packages organized in a special directory tree which contains a few additional files containing indexes and checksums for the packages. If a user adds a repository to the \textbf{/etc/apt/sources.list} file, all the packages available can be viewed and installed as if they were the packages contained in \textit{Debian}.\cite{aboutdebian}

A complete list of the current ``stable'' \textit{Debian} packages is located at \url{http://packages.debian.org/stable/}. Currently, FEUP also provides this list at \url{http://mirrors.fe.up.pt}.

The \textit{apt}-utilities can retrieve the packages from DVDs or the Internet via \textit{HTTP} or \textit{FTP}, which means that a system can be fully updated via an Internet connection.

There are several guides on how to install and manage a \textit{Debian} repository, so the process will not be described in this document, as the information would be redundant and time-wasting. Some of these guides can be found at~\cite{debian-config,linux-config}.

\section{ISO Images}\label{iso}

An ISO file is and ``image'' of an entire CD or DVD. All the contents of a disc can be represented perfectly in a single ISO file. They are often used to distribute large programs over the Internet as all the files can be contained in one single file - the ISO~\cite{about-iso}.

These are the files that will be created when using the web portal discussed earlier in this document. 

There are several tools available for creating such files - Symantec's GHOST (\ref{iso:ghost}), Suse Studio (\ref{iso:studio}) and \textit{Ubuntu}'s VM Builder (\ref{iso:ubu-vmbuilder}).

\subsection{Symantec's GHOST}\label{iso:ghost}

GHOST\footnote{General Hardware-Oriented System Transfer} is a disk cloning program distributed by Symantec. It is the tool used by CICA to create its custom Linux distributions that are installed in FEUP's computers. \cite{symantec-ghost-tutorials}

It supports a process called ``hot imaging'', which allows the user to create images from a `LIVE' operating system and still be able to change the files.
It combines the GHOST executable file and the image file it creates (with a `.gho' extension). and gives the user the ability to restore the system to a working configuration in minutes.
A tutorial on creating a GHOST image can be found in~\cite{radified}.


\subsection{Suse Studio}\label{iso:studio}
...
SUSE Studio offers a series of utilities, all of them web-based. It is possible to specify where to get the software from (i.e. choosing the software repositories) and which type of image it is (VMWare, KVM, Live CD, XEN image, etc.)~\cite{suse-help, lifehacker-suse}.

\subsection{Ubuntu's VM Builder}\label{iso:ubu-vmbuilder}

VMBuilder is a Python-based software package for creating VM images of free software GNU/Linux-based operating systems. It is under development by the \textit{Ubuntu} Virtualization Team\footnote{https://launchpad.net/vmbuilder} and it simplifies the process of building and installing VMs~\cite{vmbuilder-ubuntu}.

VMBuilder currently supports KVM, XEN and VMWare as hypervisors. Command line options can be used to add and remove packages, choose which version of \textit{Ubuntu} to run, which mirror to use, among others~\cite{help-ubuntu-vmbuilder}.

A tutorial for working with VMBuilder can be found at~\cite{vmbuilder-debian}.
